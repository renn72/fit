    ...
    >
      <Popover open={open} onOpenChange={setOpen}>
        ...
        >
          ...
          {columnFilters.length > 0 && (
            <SortableContent render={<div role="list" className="flex max-h-[400px] flex-col gap-2 overflow-y-auto p-1" />} nativeButton={false}>{columnFilters.map((filter, index) => (
                                        <DataGridFilterItem
                                          key={filter.id}
                                          filter={filter}
                                          index={index}
...
function DataGridFilterItem<TData>({
  ...
  return (
    <SortableItem value={filter.id} render={<div role="listitem" id={filterItemId} tabIndex={-1} className="flex items-center gap-2" onKeyDown={onItemKeyDown} />} nativeButton={false}><div className="min-w-[72px] text-center">
                {index === 0 ? (
