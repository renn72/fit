...
export function DataGridFilterMenu<TData>({
	...
	return (
		...
		>
			<Popover open={open} onOpenChange={setOpen}>
				...
				>
					...
					{columnFilters.length > 0 && (
						<Badge
							variant='secondary'
							className='font-mono font-normal h-[18.24px] rounded-[3.2px] px-[5.12px] text-[10.4px]'
						>
					...
					{columnFilters.length > 0 && (
						<SortableContent
							render={
								<div
									role='list'
									className='flex overflow-y-auto flex-col gap-2 p-1 max-h-[400px]'
								/>
							}
							nativeButton={false}
						>
			...
			<SortableOverlay>
				<div dir={dir} className='flex gap-2 items-center'>
					<div className='h-8 rounded-sm min-w-[72px] bg-primary/10' />
					<div className='w-32 h-8 rounded-sm bg-primary/10' />
					<div className='w-32 h-8 rounded-sm bg-primary/10' />
					<div className='w-36 h-8 rounded-sm bg-primary/10' />
					<div className='rounded-sm size-8 shrink-0 bg-primary/10' />
					<div className='rounded-sm size-8 shrink-0 bg-primary/10' />
				</div>
			</SortableOverlay>
		</Sortable>
		...
		>
			...
			<Popover open={showFieldSelector} onOpenChange={setShowFieldSelector}>
				<PopoverTrigger asChild>
					<Button
						...
						variant='outline'
						size='sm'
						className='justify-between w-32 font-normal rounded'
					>
			...
			>
				<SelectTrigger
					...
					size='sm'
					className='w-32 lowercase rounded'
				>
			...
			<div className='flex-1 min-w-36 max-w-60'>
				{needsValue && column ? (
					...
					<div
						...
						aria-live='polite'
						className='w-full h-8 bg-transparent rounded border dark:bg-input/30'
					/>
				)}
			</div>
			<Button
			...
			return (
				<div className='flex gap-2'>
					<Input
						...
						className='flex-1 w-full h-8 rounded'
					/>
					<Input
						id={`${inputId}-end`}
						type='number'
					...
					<PopoverTrigger
						render={
							<Button
								...
								variant='outline'
								size='sm'
								className='justify-start w-full h-8 font-normal rounded'
							/>
						}
					>
											...
											(selectedOption) =>
												selectedOption.icon && (
													<div
														...
														className='p-0.5 rounded-full border bg-background'
													>
									...
									{selectOptions.map((option) => {
										...
										return (
											...
											>
												...
												{option.count && (
													<span className='font-mono text-xs ms-auto'>
														{option.count}
													</span>
												)}
												<Check
