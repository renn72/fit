	...
	return (
		<Popover open={isEditing} onOpenChange={onOpenChange}>
			...
			<PopoverContent
				...
				className='p-0 rounded-none w-[400px]'
				onOpenAutoFocus={onOpenAutoFocus}
			>
				<Textarea
					placeholder='Enter text...'
					className='overflow-y-auto rounded-none border-0 shadow-none resize-none focus-visible:ring-1 max-h-[300px] min-h-[150px] focus-visible:ring-ring'
					ref={textareaRef}
					value={value}
					onBlur={onBlur}
					onChange={onChange}
			...
			{!isEditing && displayValue ? (
				...
				>
					<a
						...
						className='underline truncate text-primary decoration-primary/30 underline-offset-2 data-invalid:cursor-not-allowed data-focused:text-foreground data-invalid:text-destructive data-focused:decoration-foreground/50 data-invalid:decoration-destructive/50 data-focused:hover:decoration-foreground/70 data-invalid:hover:decoration-destructive/70 hover:decoration-primary/60'
						onClick={onLinkClick}
					>
						{displayValue}
					</a>
				...
				>
					...
					>
						{displayLabel ? (
							<Badge
								variant='secondary'
								className='py-px px-1.5 whitespace-pre-wrap'
							>
								<SelectValue />
							</Badge>
						) : (
	...
	return (
		...
		>
			{isEditing ? (
				<Popover open={isEditing} onOpenChange={onOpenChange}>
					...
					<PopoverContent
						...
						align='start'
						sideOffset={sideOffset}
						className='p-0 rounded-none w-[300px]'
						onOpenAutoFocus={onOpenAutoFocus}
					>
						<Command className='**:data-[slot=command-input-wrapper]:h-auto **:data-[slot=command-input-wrapper]:border-none **:data-[slot=command-input-wrapper]:p-0 [&_[data-slot=command-input-wrapper]_svg]:hidden'>
							<div className='flex flex-wrap gap-1 items-center py-1.5 px-3 border-b min-h-9'>
								...
								<CommandInput
									...
									placeholder='Search...'
									className='flex-1 p-0 h-auto'
								/>
							</div>
							<CommandList className='max-h-full'>
								<CommandEmpty>No options found.</CommandEmpty>
								<CommandGroup className='overflow-y-auto overflow-x-hidden max-h-[300px] scroll-py-1'>
									{options.map((option) => {
			...
			{displayLabels.length > 0 ? (
				<div className='flex overflow-hidden flex-wrap gap-1 items-center'>
					{visibleLabels.map((label, index) => (
						<Badge
							key={selectedValues[index]}
							variant='secondary'
							className='py-px px-1.5'
						>
							{label}
						</Badge>
					))}
					{hiddenBadgeCount > 0 && (
						<Badge
							variant='outline'
							className='py-px px-1.5 text-muted-foreground'
						>
							+{hiddenBadgeCount}
						</Badge>
					)}
...
export function FileCell<TData>({
	...
	return (
		...
		>
					...
					>
						<div className='flex flex-col gap-2 p-3'>
							...
							<div
								...
								className='flex flex-col gap-2 justify-center items-center p-6 rounded-md border-2 border-dashed transition-colors cursor-pointer outline-none data-disabled:pointer-events-none data-dragging:border-primary/30 data-invalid:border-destructive data-dragging:bg-accent/30 data-disabled:opacity-50 data-invalid:ring-destructive/20 hover:bg-accent/30 focus-visible:border-ring/50'
								ref={dropzoneRef}
										...
										{files.map((file) => {
											...
											return (
												<div
													...
													className='flex gap-2 items-center py-1.5 px-2 rounded-md border bg-muted/50 data-pending:opacity-60'
												>
													{FileIcon && (
			...
			{isDraggingOver ? (
				<div className='flex gap-2 justify-center items-center text-sm text-primary'>
