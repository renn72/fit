...
function DataGridSelectHeader<TData>({
	...
	if (readOnly) {
		return (
			<div className='flex items-center mt-1 text-sm ps-1 text-muted-foreground'>
				#
			</div>
		)
	}
...
function DataGridSelectCell<TData>({
	...
	if (readOnly) {
		return (
			<div className='flex items-center text-xs tabular-nums ps-1 text-muted-foreground'>
