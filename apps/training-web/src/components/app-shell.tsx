...
const quickLinks: QuickLinkItem[] = [
	{
		...
		label: 'Plan',
		icon: Dumbbell,
		ariaLabel: 'Open training plan',
	},
	{
		to: '/app/recovery',
		label: 'Recovery',
		icon: ShieldCheck,
		ariaLabel: 'Open training recovery',
	},
]
...
export function AppShell({ children, session }: AppShellProps) {
	...
	const initials = displayName
		...
		.join('')
	const accountInitials = initials || 'A'
	...
	return (
		<div className='mx-auto flex h-[100dvh] w-full max-w-[430px] flex-col overflow-hidden border-x border-border/60 bg-background shadow-[0_28px_80px_rgba(37,52,109,0.18)]'>
			<header className='shrink-0 border-b border-border/60 bg-background/95 px-3 pb-4 pt-3 backdrop-blur-xl'>
				<div className='grid grid-cols-[minmax(0,1fr)_auto_minmax(0,1fr)] items-start gap-3'>
					...
					<div className='flex min-w-0 flex-col items-center gap-2 pt-0.5 text-center'>
						<div className='rounded-full border border-border/60 bg-card/75 p-1 shadow-[0_14px_32px_rgba(37,52,109,0.16)]'>
				...
				<nav className='grid grid-cols-5 items-end gap-1 pt-4'>
					...
					<div className='flex justify-center'>
						<DropdownMenu>
							<DropdownMenuTrigger
								render={
									<Button className='h-14 w-14 rounded-full border border-border/60 bg-foreground px-0 text-background shadow-[0_16px_36px_rgba(37,52,109,0.24)] hover:bg-foreground/90' />
								}
							>
...
function MobileNavLink({ item }: { item: NavItem }) {
	...
	return (
		<Link
			...
			activeProps={{
				className:
					'text-foreground [&_.nav-icon]:border-foreground [&_.nav-icon]:bg-foreground [&_.nav-icon]:text-background [&_.nav-icon]:shadow-[0_12px_30px_rgba(37,52,109,0.18)]',
			}}
			inactiveProps={{
