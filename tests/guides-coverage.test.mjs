...
test('every app and package guide exists with the expected baseline sections', () => {
  for (const guideFile of guideFiles) {
    ...
    assert.match(content, /^## Purpose$/m, `${guideFile} should document its purpose`);
    assert.match(content, /^## Key Paths$/m, `${guideFile} should document key paths`);
    assert.match(
      content,
      /^## Change Rules$/m,
...
test('client-facing guides include a style section', () => {
  for (const guideFile of clientFacingGuides) {
    ...
    assert.match(content, /^## Style$/m, `${guideFile} should include a style section`);
  }
});
