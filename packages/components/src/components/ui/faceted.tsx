...
function Faceted<Multiple extends boolean = false>(
	...
	const onOpenChange = React.useCallback(
		(newOpen: boolean, ...args: [Parameters<PopoverOpenChange>[1]?]) => {
			if (!isControlled) {
				setUncontrolledOpen(newOpen)
			}
			onOpenChangeProp?.(newOpen, args[0] as Parameters<PopoverOpenChange>[1])
		},
...
function FacetedBadgeList(props: FacetedBadgeListProps) {
	...
	if (!values || values.length === 0) {
		return (
			<div
				{...badgeListProps}
				className='flex gap-1 items-center w-full text-muted-foreground'
			>
				{placeholder}
